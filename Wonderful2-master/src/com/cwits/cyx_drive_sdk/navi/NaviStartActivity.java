...
public class NaviStartActivity extends Activity implements OnClickListener,
	...
	private int calculateDriverRoute() {
		...
		case BY_MAP_POSITION:
			...
			Log.w("lxh", "mStartPoints = " + mStartPoints.get(0).getLatitude()
					+ " " + mStartPoints.get(0).getLongitude() + "mEndPoints = "
					+ mEndPoints.get(0).getLatitude() + " "
					+ mEndPoints.get(0).getLongitude() + "mWayPoints = "
					+ mWayPoints.get(0).getLatitude() + " "
					+ mWayPoints.get(0).getLongitude()+" driveMode= "+driveMode);
	...
	private void initView(Bundle savedInstanceState) {
		...
		ArrayAdapter<String> strategyAdapter = new ArrayAdapter<String>(
				this,R.layout.strategy_inputs,
				mStrategyMethods);
		mStrategyText.setAdapter(strategyAdapter);

		mPositionAdapter = new ArrayAdapter<String>(
				this,R.layout.strategy_inputs,
				mPositionMethods);
	...
	private void initResources() {
		...
		mPositionMethods = new String[] {
				res.getString(R.string.mypoistion),
	...
	private void initMapAndNavi() {
		...
		mWayMarker = mAmap
				.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory
						.fromBitmap(BitmapFactory.decodeResource(
								getResources(),  R.drawable.way))));
		mEndMarker = mAmap
				.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory
						.fromBitmap(BitmapFactory.decodeResource(
								getResources(),  R.drawable.end))));
	...
	public void onClick(View v) {
		...
		// 模拟导航处理事件
		else if (v.getId() == R.id.navi_navi_button) {
			mNaviMethod = NAVI_METHOD;
			calculateRoute();
		}
		// 起点点击事件
		else if (v.getId() == R.id.navi_start_image
				|| v.getId() == R.id.navi_start_edit) {
		...
		// 途经点点击事件
		else if (v.getId() == R.id.navi_way_image
				|| v.getId() == R.id.navi_way_edit) {
		...
		// 终点点击事件
		else if (v.getId() == R.id.navi_end_image
				|| v.getId() == R.id.navi_end_edit) {
		...
		// 策略点击事件
		else if (v.getId() == R.id.navi_strategy_image
				|| v.getId() == R.id.navi_strategy_edit) {
